...
| Project-contact publication state | `recorded-not-published` |
...
| Status | Approved-disabled; public project-contact publication and verification remain prerequisites |
...
- Bluesky's developer guidelines prohibit spam, require current public contact
  ...
  `cyberspaceradio@proton.me`, now recorded in the project
  [contact file](../../CONTACT.md), but its presence in this local, untracked
  repository is not public publication and does not yet clear the live-trial
  prerequisite. The intended GitHub repository existed but was empty when this
  row was updated; after the file is pushed, its raw URL must be fetched without
  authentication and checked for the exact mailto route before the publication
  status changes.
...
- The source owner's `support@bsky.app` address is not the project operator's
  ...
  `mailto:cyberspaceradio@proton.me`. Until the project contact is published on
  a reachable public page, its mailbox receipt is checked, and the trial client
  exposes it, the Jetstream row remains approved-disabled and live Jetstream
